...
export default {
	...
	getters: {
			...
			return state.Params[JSON.stringify(params)] ?? {}
				...
				getProperty: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.Property[JSON.stringify(params)] ?? {}
		},
				getPropertyAll: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
	...
	actions: {
		...
		async QueryProperty({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				...
				throw new SpVuexError('QueryClient:QueryProperty', 'API Node Unavailable. Could not perform query: ' + e.message)
		...
		async QueryPropertyAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				...
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryPropertyAll({...query, 'pagination.key':(<any> value).pagination.next_key})).data
				...
				throw new SpVuexError('QueryClient:QueryPropertyAll', 'API Node Unavailable. Could not perform query: ' + e.message)
